...
title Convert Videos to MP4 (Muted)
...
for %%E in (%exts%) do (
    if /I "%ext%"=="%%E" (
        ...
        echo 🔇 Muting and converting: %filename%
        "%ffmpeg_exe%" -i "%filepath%" -an -c:v libx264 -preset slow -crf 23 "%outdir%\%nameonly%_muted.mp4"
    )
)
goto :eof
...
:end
echo.
echo ✅ Done! Muted MP4s saved in 'converted_mp4' folders
pause
